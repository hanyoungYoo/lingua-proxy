...
- `max_output_tokens_for_translation` (default 4000). Translation cost scales
  with the length of the reply while the saving does not, so a request asking
  for a very long answer is now passed through untranslated instead of quietly
  costing more than it saves. Measured on a Sonnet-class model: a bounded answer
  saves ~49%, an open-ended one costs ~40% more.
